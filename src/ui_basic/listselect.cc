...
inline const UI::PanelStyleInfo* BaseListselect::background_style() const {
	return (selection_mode_ & ListselectLayout::kDropdown) != 0 ?
...
int BaseListselect::get_lineheight() const {
	return get_lineheight_without_padding() +
	       ((selection_mode_ & ListselectLayout::kDropdown) != 0 ? 2 * kMargin : kMargin);
}
...
int BaseListselect::calculate_desired_width() {
	...
	const int chkw = (selection_mode_ & ListselectLayout::kShowCheck) != 0 ? check_pic_->width() + 10 : 0;
...
void BaseListselect::layout() {
	...
	if (scrollbar_.is_enabled()) {
		...
		if ((selection_mode_ & ListselectLayout::kDropdown) != 0) {
...
void BaseListselect::draw(RenderTarget& dst) {
	// draw text lines
	const int eff_h =
	   (selection_mode_ & ListselectLayout::kDropdown) != 0 ? get_inner_h() - kMargin : get_inner_h();
	...
	int chkw = (selection_mode_ & ListselectLayout::kShowCheck) != 0 ? check_pic_->width() + 10 : 0;
	...
	while (idx < entry_records_.size()) {
		...
		Vector2i point((selection_mode_ & ListselectLayout::kDropdown) != 0 ? 3 : 1, y);
		uint32_t maxw =
		   get_eff_w() -
		   ((selection_mode_ & ListselectLayout::kDropdown) != 0 ? scrollbar_.is_enabled() ? 4 : 5 : 2);
